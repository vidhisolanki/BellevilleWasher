...
\label{fig:Onewasher}}
\hfil
\subfloat[Example of a stack with series-parallel arrangement]
{\raisebox{1.2em}{\includegraphics[scale=1]{Photo_2units2U2D.eps}%
...
The force displacement response of a washer stack is obtained here by applying cyclic compressive loading to the stack in a materials testing machine. At first, this was done by simply compressing the stack between loading plates as shown in Figure \ref{fig:MTSstack1}. The resulting force-displacement curves of Figure \ref{fig:K187522U2DMTSnoLWA} showed changes in slope that weren’t observed in force-displacement measurements of the same stack in shake table tests (the special instrument designed to measure this response in situ under the equipment in the shake table test is discussed in section \ref{sec:shaketableexperiments}). This is likely because of slight misalignment between washers in the stack initially that corrects itself during the loading. To avoid this, an alternative approach was devised, where the compressive loading was applied to the stack through a loading frame and guiding rod as illustrated in Figure \ref{fig:MTSstackLWA1}. This arrangement is closer to how the washer stack is installed under the equipment. The guiding rod prevents any initial misalignment and subsequent realignment. The resulting force-displacement measurement, shown in Figure \ref{fig:K187522U2DMTSLWA}, does not have any unexpected changes in slope. Indeed the in situ measurements from the shake table tests follow these curves closely as seen in Figure \ref{fig:K18753MTSvsST}. Figure \ref{fig:K1750MTS} shows force-displacement curves for two configurations of the K1750-J-057 washers. They exhibit the reduction in stiffness with loading noted in Figure \ref{fig:Forcedisponewasher} due to their higher $h$/$t$ ratio.\\
\begin{figure}
\centering
\subfloat[Washer stack compressed directly 
between loading plates]{\raisebox{3.5em}{\includegraphics[scale=0.65]{MTSstack.eps}%
\label{fig:MTSstack1}}}
\hfil
\subfloat[Measured force-displacement hysteresis
]{\includegraphics[scale=0.6]{K187522U2D_MTS_noLWA.eps}%
\label{fig:K187522U2DMTSnoLWA}}
\caption{Washer stack force-displacement response measurement by \textit{compressing the stack directly} in the materials testing machine; force-displacement response exhibits unexpected changes in slope.}
\label{fig:K187522U2D_MTS_noLWA}
\end{figure}

\begin{figure}
\centering
\subfloat[Washer stack compressed through a 
loading frame with guiding rod]{\raisebox{3.5em}{\includegraphics[scale=0.65]{MTSstackLWA.eps}%
\label{fig:MTSstackLWA1}}}
\hfil
\subfloat[Measured force-displacement hysteresis
]{\includegraphics[scale=0.6]{K187522U2D_MTS_LWA.eps}%
\label{fig:K187522U2DMTSLWA}}
\caption{Washer stack force-displacement response measurement by \textit{compressing the stack through a loading frame with guiding rod}; this is closer to how to washers are mounted under the equipment, and the force-displacement response is clean.}
\label{fig:MTSstackLWA}
\end{figure}

%%%%%%%%%% Figure 3-3
\begin{figure}
\centering
\subfloat[2 units of 2U2D K1875-G-086]{\includegraphics[scale=0.6]{K187522U2D_MTSvsST.eps}%
\label{fig:K187522U2DMTSvsST}}
\hfil
\subfloat[3 units of 3U3D K1875-G-086
]{\includegraphics[scale=0.6]{K187533U3D_MTSvsST.eps}%
\label{fig:K187533U3DMTSvsST}}
\caption{Measured force-displacement curves for two configurations. The legend MTS denotes measurement from the materials testing machine through the frame with guiding rod. The other curves are in situ measurements from the shake table a different preload levels. The in-situ measurements follow the testing machine results closely, confirming that the washer stack response can be characterized fully using simply a materials testing machine.}
\label{fig:K18753MTSvsST}
\end{figure}

%%%%%%%%%% Figure 3-4
\begin{figure}
\centering
\subfloat[2 units of 2U2D K1750-J-057]{\includegraphics[scale=0.6]{K175022U2D_MTS.eps}%
\label{fig:K175022U2DMTS}}
\hfil
\subfloat[2 units of 3U3D K1750-J-057
]{\includegraphics[scale=0.6]{K175023U3D_MTS_withLWA.eps}%
\label{fig:K175023U3DMTSwithLWA}}
\caption{Measured force-displacement curves for two configurations with the K1750-J-057 washers. They have a softening behavior because of their larger $h$/$t$ ratio (cf. Figure \ref{fig:Forcedisponewasher})}
\label{fig:K1750MTS}
\end{figure}
...
\begin{figure}
\centering
\subfloat[]{\includegraphics[scale=0.6]{k2120_X_freevib.eps}%
\label{fig:k2120Xfreevib}}
\hfil
\subfloat[]{\includegraphics[scale=0.6]{CVTonspringsFBD.eps}%
\label{fig:CVTonspringFBD}}
\caption{Acceleration measured when CVT is mounted on springs and the resulting frequency fit, and process to compute CVT mass moment of inertia from measured frequency; the dimension b is 6.375in in the $X$ direction and 9.25in in the $Y$ direction.}
\label{fig:CVTonspringAnalysis}
\end{figure}
...
& & 500lb (25\%) \\
& & 1000lb (50\%) \\ \midrule
\multirow{2}{*}{K1750-J-057} & \multirow{2}{*}{3 units of 3U3D} & 500lb (17\%) \\
& & 1500lb (50\%) \\
\bottomrule
%\multicolumn{3}{l}{\small{$^1$Obtained from \cite{kote2019model}}}
\end{tabular}
\end{center}
\end{table}
For each configuration, a sequence of tests consisting of low-level broad-band random input (for baseline system identification) and the CERL ground motion with increasing amplitude levels (culminating at 0.5g) were applied first in the X direction and then in the Y direction. The 5\%-damped test response spectra of the applied motion are shown in Figure \ref{fig:TRSRRS005}.

\begin{figure}
    \centering
    \includegraphics[scale=0.5]{TRSRRS005.eps}
    \caption{5\%-damped test response spectra of applied CERL motion compared with required response 
spectrum}
    \label{fig:TRSRRS005}
\end{figure}
\subsection{Correlating measured forces and accelerations}
Various measured forces and accelerations are correlated to gain confidence in the measurements to then compare with analysis predictions in section \ref{sec:nonlinmodelanalysis}. Consider the free body diagram in Figure \ref{fig:FBDCut1}. The dimensions used in this free body diagram are summarized in Table \ref{tab:CVTinertiaFBD}. The equilibrium equations
\begin{equation}\label{eqn:FBDs}
  \begin{aligned} 
  \sum F_X = & 0 \implies V = m\xtddot \\
  \sum M = 0 \implies \Mspool = & -\Izero\ddot{\theta}-VL = -\Izero\ddot{\theta}-m\xtddot L
\end{aligned}
\end{equation}
The correlation between the left and right hand sides of the second of equations ~\eqref{eqn:FBDs}, each of which is obtained using independent measurements, is verified in Figure 4-8, demonstrating consistency.
\begin{figure}
    \centering
    \includegraphics[scale=0.6]{FBDCut1.eps}
    \caption{Free body diagram with mid-spool cut}
    \label{fig:FBDCut1}
\end{figure}

\begin{table}[ht]
\caption{\label{tab:CVTinertiaFBD} Summary of CVT inertia properties}
\begin{center}   
\begin{tabular}{cm{9.5cm}c} 
 \toprule
Dimension & Description & Value (in)\\ \midrule
$r$ & Distance between CVT base and mid spool & 11 \\ \midrule
$L$ & Distance between CVT center of mass and mid spool & 46-1/16 \\ \midrule
$l$ & Distance between CVT center of mass and center of rotation; this 
is obtained by fitting the equation $\ddot{x}_\text{g}+\ddot{\theta}=\xtddot$ & 38 \\ 
\bottomrule
\end{tabular}
\end{center}
\end{table}

Next consider the free body diagrams shown in Figure \ref{fig:FBDCuts23}. In this figure, $F_{i\text{t}}$ and $F_{i\text{b}}$ stand for the force in the top and bottom washer stacks at corner $i$, and $F_i$ for the force in the corresponding guide rod. Superscript ``pre'' denotes the preload component of these forces and $\Delta$ denotes the change due to the CVT weight and during dynamic response. Since each assembly of washers under preload is in equilibrium,
\begin{equation}\label{eqn:preload}
  F^{\text{pre}}_{i\text{b}}=F^{\text{pre}}_{i\text{t}}=F^{\text{pre}}_{i}
\end{equation}
Furthermore,
\begin{equation}\label{eqn:preload2}
  F^{\text{pre}}_{\text{1}}=F^{\text{pre}}_{\text{2}}=F^{\text{pre}}_{\text{3}}=F^{\text{pre}}_{\text{4}}
\end{equation}
since all the assemblies are preloaded to the same level. Summing moments about the center of rotation $O$, 
\begin{equation}
\begin{split}
  & \Mspool - (F^{\text{pre}}_{\text{1}} + \Delta F_{\text{1}}+ F^{\text{pre}}_{\text{4}} + \Delta F_{\text{4}})b  + (F^{\text{pre}}_{\text{2}} + \Delta F_{\text{2}}+ F^{\text{pre}}_{\text{3}} + \Delta F_{\text{3}})b  \\
  & + (F^{\text{pre}}_{\text{1t}} - \Delta F_{\text{1t}}- F^{\text{pre}}_{\text{4t}} + \Delta F_{\text{4t}})b - (F^{\text{pre}}_{\text{2t}} - \Delta F_{\text{2t}}- F^{\text{pre}}_{\text{3t}} + \Delta F_{\text{3t}})b - Vr= 0
\end{split}
\end{equation}
Using equation \eqref{eqn:preload} to cancel out the $F^{\text{pre}}_{i}$ and $F^{\text{pre}}_{i\text{t}}$, and substituting for $V$ from equation \eqref{eqn:FBDs},
\begin{equation}\label{eqn:Mspool}
  \Mspool-(\Delta F_\text{1}+\Delta F_\text{4})b + (\Delta F_\text{2}+\Delta F_\text{3})b - (\Delta F_\text{1t}+\Delta F_\text{4t})b - (\Delta F_\text{2t}+\Delta F_\text{3t})b - m\xtddot r = 0
\end{equation}
A further assumption is made that
\begin{equation}
  \Delta F_i = \Delta F_{i\text{b}} = \Delta F_{i\text{t}} 
\end{equation}
This does not follow from equilibrium, but is taken to be true based on symmetry/compatibility of top and bottom stack deformations. Then equation \eqref{eqn:Mspool} becomes
\begin{equation}
  \Mspool=2(\Delta F_\text{1}+\Delta F_\text{4} - \Delta F_\text{2}-\Delta F_\text{3})b+m\xtddot r
\end{equation}
Using equation \eqref{eqn:preload2}, this can be further written as
\begin{equation}
  \Mspool=\underbrace{2(F_\text{1}-F_\text{2} -F_\text{3}+F_\text{4})b}_{\MCVT}+m\xtddot r
\end{equation}
where $F_i=F^{\text{pre}}_{i}+\Delta F_i$ is the directly the force measured by LWA $i$. Verifying this relationship, which again involves multiple independent measurements, adds further confidence in the 
measured data for comparison with analysis. In particular, this confirms the thinking that the in-situ behavior of the washer stacks can be determined from the materials testing machine measurements. This comparison is shown in Figure \ref{fig:FBDCut4}. A final free body diagram, Figure \ref{fig:FBDCut4}, is considered in relation to the insulator base moment. The mass and mass moment of inertia of the insulator alone are not known, however its center of mass is assumed to be at mid-height. Summing the moments about the insulator base,
\begin{equation}
  M_{\text{ins}} = -I_{\text{0ins}}\ddot{\theta}-m_{\text{ins}}\ddot{x}_{\text{t,ins}}L_1
\end{equation}
Substituting the kinematics, $\ddot{x}_{\text{t,ins}} = \ddot{x}_{\text{g}}+L_2\ddot{\theta}$,
\begin{equation}\label{eqn:Mins}
M_{\text{ins}} = - \underbrace{(I_{\text{0ins}}+m_{\text{ins}}L_\text{1}L_\text{2})}_{I_\text{ins}} \ddot{\theta} - m_{\text{ins}}L_\text{1}\ddot{x}_{\text{g}}
\end{equation}
The insulator mass, $m_\text{ins}$, and the net mass moment of inertia, $I_\text{ins}$ are known. Equation \eqref{eqn:Mins} is 
used to fit them to measure data. The fit values are $m_{\text{ins}}=$360/386.4lb-s$^2$/in (about 40\% of the total mass of the CVT) and $I_\text{ins}=$3,500lb-in-s$^2$. These values are used when comparing analysis results to measured insulator base moment. The measured insulator base moments are shown in Figure 4-12 and Figure 4-13.

\begin{figure}
    \centering
    \includegraphics[scale=0.6]{FBDCuts23.eps}
    \caption{Free body diagrams relating moment measured at the spool with moment estimated using LWAs}
    \label{fig:FBDCuts23}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[scale=0.6]{FBDCut4.eps}
    \caption{Free body diagram used to relate insulator base moment to measured accelerations; $L_1$ is the mid-height of the insulator, $L_1=$in; $L_2$ is the distance from the center of rotation to the center of mass of the insulator, $L_2=$in.}
    \label{fig:FBDCut4}
\end{figure}

\section{Nonlinear modeling and analysis}\label{sec:nonlinmodelanalysis}
In this section, a nonlinear model is developed for the CVT installed on Belleville washer stacks. The CVT is modeled as a rigid body, and a hysteretic model is used to represent the moment-rotation behavior resulting from the aggregate action of the washer stacks (the type of moment-rotation hysteretic behavior seen in the measurements of Figure 4-11).
\subsection{From stack force-displacement to system moment-rotation}
The moment-rotation hysteresis behavior can be obtained from the washer stack force-displacement response, whose measure was discussed in section \ref{sec:washerstackforcedisp}, using the process depicted in Figure 5-1. An example of this process is illustrated in Figure 5-2. The specific example shown in Figure 5-2 corresponds to a low preload (25\% of flattening). In this condition, when the washer stacks on one side reach the highest deformation (bottom left of Figure 5-2), the stacks on the other side relax entirely, losing preload (bottom right of Figure 5-2). The net effect is a softening of the moment-rotation behavior near the extremes (top right of Figure 5-2).

\subsection{Modeling moment-rotation hysteresis}
The moment-rotation behavior decomposed into two parallel components – one elastic and one hysteretic as illustrated in Figure \ref{fig:K187550percentmeasuredhysteresis}. The split is given by
\begin{equation}\label{eqn:MCVT}
  \MCVT=K\thetaw+M_\text{HYS}
\end{equation}
for cases with sufficient preload as in Figure 5-3(a) and by
\begin{equation}\label{eqn:MCVT2}
  \MCVT=K \text{tanh}(\alpha\thetaw)+M_\text{HYS}
\end{equation}
for cases with insufficient preload as in Figure 5-3(b), the tanh function capturing the S shape. The hysteretic component is represented using what is commonly referred to as the Bouc-Wen model \cite{bouc1967forced, wen1976method} (for a description of this model using notation similar to that used here, see \cite{sivaselvan2000hysteretic, sivaselvan2004nonlinear}, and is given by
\begin{equation}\label{eqn:MHYS}
  \dot{M}_\text{HYS}=k\left(1- \left| \frac{M_\text{HYS}}{M_\text{yield}}\right|^N (\eta + (1-\eta)sgn(M_\text{HYS}\dot{\theta}_\text{w})) \right)\dot{\theta}_\text{w}
\end{equation}
where $M_\text{yield}$ is the yield moment, and $N$ and $\eta$ are parameters that control the shape of the hysteresis. For an explanation of these parameters, see \cite{constantinou1987dynamics}. Figure \ref{fig:K187550percentmodelmeasuredhysteresis} shows a comparison of the model with the measured moment-rotation behavior when the model is driven by the measured rotation of the CVT relative to the spool plate ($\thetaw$). In constructing the full system model, the flexibility of the spool plate must be incorporated as well; this is recognized when comparing the magnitudes of the plate deformation (see Table \ref{tab:deriveresponsequant}), washer deformation and total rotation of the CVT in Figure 5-5.

\begin{figure}
\centering
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMCVTthetaw0.5measuredN002}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5measuredN0022}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmeasuredN0023}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X25percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMCVTthetaw0.5measuredN0024}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X25percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5measuredN00225}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X25percentpreload_MCVT_thetaw_0.5g_measured.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmeasuredN0026}}
\caption{Parallel decomposition of moment-rotation behavior into elastic and hysteretic components (example shown for 2 units of 2U2D).}
\label{fig:K187550percentmeasuredhysteresis}
\end{figure}

\begin{figure}
\centering
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MCVT_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMCVTthetaw0.5gmodelmeasuredN002}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MHYS_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmodelmeasuredN002}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MHYS_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmodelmeasuredN0022}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MCVT_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMCVTthetaw0.5gmodelmeasuredN0023}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MHYS_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmodelmeasuredN0024}}
\hfil
\subfloat{\includegraphics[scale=0.4]{k187522u2d_X50percentpreload_MHYS_thetaw_0.5g_modelmeasured_N002.eps}%
\label{fig:k187522u2dXMHYSthetaw0.5gmodelmeasuredN00225}}
\caption{Comparison on model and measured moment-rotation behavior when the model is driven by the measured $\thetaw$ as input (example shown for 2 units of 2U2D).}
\label{fig:K187550percentmodelmeasuredhysteresis}
\end{figure}

\subsection{Summary of nonlinear dynamic model}
Combining the moment-rotation model of equations \eqref{eqn:MCVT} and \eqref{eqn:MCVT2} or \eqref{eqn:MHYS} with the equation of motion of the CVT (considered as a rigid body), and including the flexibility of the spool plate results in the full nonlinear model of the Belleville washer-protected CVT:
\begin{equation}\label{eqn:EOMs}
  \begin{aligned} 
  \text{Equation of motion of CVT: } (\Izero+ml^2)\ddot{\theta}+\MCVT = & 0 ml\ddot{x}_\text{g} \\
  \text{Hysteretic model (equation \eqref{eqn:MHYS} rewritten): } \dot{M}_\text{HYS}= & k(1-H_1H_2)\dot{\theta}_\text{w}
\end{aligned}
\end{equation}
where $H_1 = \left| \frac{M_\text{HYS}}{M_\text{yield}}\right|^N$, $H_2=(\eta + (1-\eta)sgn(M_\text{HYS}\dot{\theta}_\text{w}))$, $\MCVT$ given either by equation \eqref{eqn:MCVT} or \eqref{eqn:MCVT2}, the spool plate deformation, $\theta_\text{p}=\frac{\MCVT}{K_\text{p}}$ ($
K_\text{p}$ p being the spool plate stiffness), $\thetaw=\theta-\theta_\text{p}$ and $\dot{\theta}_\text{w}=\left(\frac{K_\text{p}}{K_\text{p}+k(1-H_1H_2)+K}\right)\dot{\theta}$ for sufficient preload and $\dot{\theta}_\text{w}=\left(\frac{K_\text{p}}{K_\text{p}+k(1-H_1H_2)+K\alpha(1-\text{tanh}^2(\alpha\thetaw))}\right)\dot{\theta}$ for insufficient preload. Solutions to the differential equations \eqref{eqn:EOMs} together with these auxiliary equations are computed using the solver \texttt{ode15s} in MATLAB \cite{MATLAB}. The insulator moment can be calculated as an output of the analysis using equation \eqref{eqn:Mins}.
\subsection{Analysis results}
As examples, analysis results are shown for 2 cases, but with the 2 units of 2U2D configuration under 0.5g CERL motion in the X direction, one case with 50\% preload and another with 25\% preload in Figure 5-6 and Figure 5-7.

\section{Equivalent linear modeling}\label{sec:equilinmodel}
In the previous section, the Belleville washer-protected CVT system was analyzed using a nonlinear dynamic procedure. In this section, an alternate approach is presented using an equivalent linear approach. This approach may be better suited for practical use as well as provides insights in terms of system frequency and damping ratio. First, an equivalent linear model obtained by fitting to experimental measurements is presented; the frequencies and damping ratios so obtained are used as reference for the second approach. In the second 
approach, equivalent stiffnesses and damping ratios are obtained graphically from the moment-rotation hysteresis curves.
\subsection{Equivalent frequency and damping ratio from measurements}\label{subsec:equistiffdamp}
An equivalent linear model can be obtained from experimental measurements by fitting a linear model to it. Here, this is done using the subspace linear system identification method \cite{van2012subspace}. Figure 6-2 shows a comparison of such a linear fit with measured responses. Table \ref{tab:subspace} summarizes equivalent frequencies and damping ratios for all the configurations tested. Approximate equivalent stiffness, frequency and damping ratio can be obtained graphically as illustrated in Figure \ref{fig:EquiStiffnessDamping}. The equivalent stiffness, $k_{\text{eq}}$, is obtained as the ratio of the difference 
between the positive and negative peak moments and the different between the peak positive and negative rotations. The frequency is then
\begin{equation}
  f = \frac{1}{2\pi}\sqrt{\frac{k_\text{eq}}{\Izero+m\hCG^2}}
\end{equation}
and the damping ratio is obtained by equating the area of the hysteresis, $A_\text{h}$, to that of a viscous damper undergoing the same peak displacement at steady state with frequency $f$ \cite{ChopraDynamics}
\begin{equation}
  \xi = \frac{A_\text{h}}{2\pi k_\text{eq}u^2_\text{0}}
\end{equation}
where these formulae parallel equations (17.2-4) and (17.2-5) of Chapter 17 of ASCE 7-16 on Seismically Isolated Structures \cite{loads2017minimum}.
\begin{figure}
\centering
\subfloat{\includegraphics[scale=0.7]{EquiStiffnessDamping.eps}%
\label{fig:EquiStiffnessDamping1}}
\hfil
\subfloat{\includegraphics[scale=0.5]{k187522u2d_X25percentpreload_MCVT_thetaw_0.5g_measured_annotated.eps}%
\label{fig:EquiStiffnessDamping2}}
\caption{Concept of graphically obtaining equivalent stiffness and damping ratio.}
\label{fig:EquiStiffnessDamping}
\end{figure}
\begin{table}[ht]
\caption{\label{tab:subspace} Summary of findings from experimental measurements and equivalent linear modeling for 0.5g CERL excitation. Damping ratios in parentheses denote those obtained by the procedure in \ref{subsec:equistiffdamp}.}
\begin{center}   
\begin{tabular}{m{2.3cm}cccccccccccc} 
 \toprule
Configuration & \multicolumn{2}{c}{\makecell{CVT fixed \\ on spool \\ (reference)}} & \multicolumn{6}{c}{\makecell{K1875-G-086\\ 2 units of 2U2D}} & \multicolumn{4}{c}{\makecell{K1875-G-086 \\3 units of 3U3D}} \\ \midrule
Preload level & \multicolumn{2}{c}{-}  & \multicolumn{2}{c}{0\%} & \multicolumn{2}{c}{25\% (500lbs)} & \multicolumn{2}{c}{50\% (1000lbs)} & \multicolumn{2}{c}{17\% (500lbs)} & \multicolumn{2}{c}{50\% (1500lbs)} \\ \midrule
Direction of excitation & $X$ & $Y$ & $X$ & $Y$ & $X$ & $Y$ & $X$ & $Y$ & $X$ & $Y$ & $X$ & $Y$ \\ \midrule
Natural frequency (Hz) & 6.9 & 9.4 & \makecell{2.9 \\ (3.0)} & \makecell{4.0\\ (4.3)} & \makecell{3.6 \\(3.5)} & \makecell{5.1\\ (5.1)} & \makecell{4.1\\ (3.8)} & \makecell{5.5\\ (5.4)} & \makecell{3.6\\ (3.6)} & \makecell{5.1\\ (5.0)} & \makecell{4.5\\ (4.7)} & \makecell{5.9\\ (5.6)} \\ \midrule
Damping ratio (\%) & 0.5 & 0.6 & \makecell{3.9 \\ (3.6)} & \makecell{3.7\\ (3.4)} & \makecell{6.5 \\(7.1)} & \makecell{6.4\\ (8.7)} & \makecell{9.9\\ (14.2)} & \makecell{8.1\\ (13.6)} & \makecell{7.8\\ (8.2)} & \makecell{7.3\\ (9.9)} & \makecell{9.3\\ (24.2)} & \makecell{6.0\\ (20.0)} \\ \midrule
Maximum insulator base moment ($\times10^4$lb-in) & 5.1 & 4.5 & 3.5 & 4.0&2.9& 3.2 & 2.4& 3.3& 2.7& 3.0& 2.4& 2.9 \\ \midrule
Maximum relative terminal displacement (in) & 0.9 & 0.6 & 3.4 & 2.1 &1.8& 1.1 & 1.4& 1.0& 1.8& 1.1& 1.2& 0.9 \\ 
\bottomrule
\end{tabular}
\end{center}
\end{table}

\subsection{Response spectrum interpretation}
It can be seen from Table 6-1 that the insulator base moment is reduced in all configurations relative to the fixed base case, and by over 50\% in the $X$ direction and by over 25\% in the $Y$ direction for the cases with sufficient preload. This however comes at the expense of greater terminal displacement, about 50\% greater for the cases with highest preload. These effects are similar to that of seismic isolation. These effects can be better understood by mapping the frequencies and damping ratios of the different configurations on a spectral plot as seen in Figure \ref{fig:ExpRRS}. For each configuration, the abscissa is the frequency and the ordinate is the spectral acceleration obtained from the equations in \cite{IEEE693} for the damping ratio of that configuration.
\begin{figure}
    \centering
    \includegraphics[scale=0.6]{ExpRRS.eps}
    \caption{Frequencies and damping ratios corresponding to different Belleville washer configurations overplotted on the 2\%- and 5\%-damped required response spectrum (RRS) from IEEE 693-2018\cite{IEEE693}.}
    \label{fig:ExpRRS}
\end{figure}
The picture conveys the relative effectiveness of the different Belleville washer configurations with respect to the reference configuration. There is a frequency reduction that for the configurations tested does not play a major role in reducing base moments. The biggest effect is the increased damping. While such deductions can be made from the figure, the main takeaway is that with the tools developed in this project, a similar evaluation can be done for any other arrangement of washers by modeling and analysis. For example, a different configuration may be designed that further reduces frequency while increasing damping, trading off lower insulator base moment with increased terminal displacement.
\section{Summary and concluding remarks}\label{sec:conclusion}

\subsection{Executive summary}\label{subsec:execsummary}
A modeling procedure has been developed to predict the seismic response of rigid equipment such as CVTs equipped with Belleville washer stacks at the base for seismic protection. The procedure allows for nonlinear dynamic analysis or an equivalent linear analysis, and is outlined in section \ref{sec:analysisprocedure}. The starting points are (a) the force-displacement hysteretic behavior of an individual stack that can be readily obtained by cyclic loading in a materials test machine, (b) inertia properties of the CVT – mass, mass moment of inertial and center of mass location. With this, the seismic response can be predicted reliably. This has been validated with experiments on an earthquake simulator (shake table). Special instrumentation was developed to obtained detailed in situ measurements of the behavior of the washer stacks in the shake table experiments. Validation using these measurements provides confidence that the analysis procedure can be used to inform decisions when installing Belleville washer devices in the field. The process also parallels procedures in ASCE 7-16 for seismic isolators. The washer stacks essentially act as seismic isolators, reducing the frequency and increasing the damping. They are effective in reducing the insulator base moment. As is the case with seismic isolation systems, reduction is base moment has to be traded off with increase in terminal displacement.
